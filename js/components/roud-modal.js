...
export class RoudMapModal {
  constructor({ elements, callbacks = {}, services = {} }) {
    ...
    this.listeners = [
      ...
      { el: 'closeBtn', event: 'click', handler: this.hideAddModal.bind(this) },
      {
        el: 'importCloseBtn',
        event: 'click',
        handler: this.hideAddModal.bind(this),
      },
      {
        el: 'promtCopyBtn',
        event: 'click',
        handler: this.handlePromtCopy.bind(this),
      },
      {
        el: 'modalCheckBox',
        event: 'change',
        handler: this.handleCheckboxChange.bind(this),
      },
      {
        el: 'uiPanel',
        event: 'click',
        handler: this.handleModalClick.bind(this),
      },
      {
        el: 'subTaskBtn',
        event: 'click',
      ...
      },
      {
        el: 'manualForm',
        event: 'click',
        handler: this.handleClearErrorManual.bind(this),
      },
      {
        el: 'importForm',
        event: 'click',
        handler: this.handleClearErrorImport.bind(this),
      },
      {
        el: 'manualSubmitBtn',
        event: 'click',
  ...
  async handleManualSubmit(e) {
    ...
    try {
      ...
      const existingNodes =
        await this.firestoreService.getElementsfromSubCollection(
          nodeData.roadmapID,
          'roadmaps',
          'nodes'
        );
      ...
      const nodeID = await this.firestoreService.addCollectionElement(
        nodeDataWithOrder,
        'roadmaps',
        'nodes'
      );
    ...
    try {
      ...
      parsedData.forEach((node, index) => {
        if (!node.title || typeof node.title !== 'string') {
          throw new Error(
            `Brak tytułu lub nieprawidłowy typ w elemencie [${index}]`
          );
        }
