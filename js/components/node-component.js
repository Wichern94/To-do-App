...
export class NodeElement {
  ...
  updateProgress() {
    const checkedCount = Array.from(this.ui.checkBoxList).filter(
      (cb) => cb.checked
    ).length;
  ...
  setupStopBtn() {
    if (!this.stopListenerBound) {
      this.ui.stopBtn?.addEventListener(
        'click',
        this.handleCompleteNode.bind(this)
      );
      ...
      for (let i = 0; i < sortedInstances.length; i++) {
        ...
        if (!currentBorder.classList.contains('hidden')) {
          if (!nextNode) {
            ...
            await this.animationManager.plumbLineAnimation(
  ...
  async getCompletedata() {
    ...
    await this.firestoreService.moveElementToFinished(
      finishedData,
      refObj,
      copyRefObj
    );
